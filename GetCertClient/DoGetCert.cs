...
-DefaultPhysicalPath='{pct}SystemDrive{pct}\inetpub\wwwroot'
...
"
                    ...
                    //// Fetch simple setup post-processing scripts.
                    //tvFetchResource.ToDisk(ResourceAssembly.GetName().Name
                    //        , String.Format("{0}{1}", Env.sFetchPrefix, lsFetchName="PostAppFolderSetup.ps1")
                    //        , loProfile.sRelativeToExePathFile(lsFetchName));
                    //tvFetchResource.ToDisk(ResourceAssembly.GetName().Name
                    //        , String.Format("{0}{1}", Env.sFetchPrefix, lsFetchName="PostAppRemoveStartup.ps1")
                    //        , loProfile.sRelativeToExePathFile(lsFetchName));
                    //tvFetchResource.ToDisk(ResourceAssembly.GetName().Name
                    //        , String.Format("{0}{1}", Env.sFetchPrefix, lsFetchName="PostAppShowStartups.ps1")
                    //        , loProfile.sRelativeToExePathFile(lsFetchName));
                    ...
                    {
                        loProfile["-Update2025-08-30"] = true;
                        loProfile.Save();
                    }
                    else
                    {
                        if ( !loProfile.bValue("-Update2025-08-30", false) )
                        {
                            // Force use of the latest version.
                ...
                DateTime    ldtMaintenanceWindowEndTime   = DoGetCert.oDomainProfile.dtValue(gsMaintWindEndKey, DateTime.MaxValue);
                            if ( DateTime.Now < ldtMaintenanceWindowBeginTime || DateTime.Now >= ldtMaintenanceWindowEndTime )
                            {
                                Env.LogIt(String.Format("    Can't continue. The \"{0}\" maintenance window is \"{1}\" to \"{2}\"."
                                                        , moProfile.sValue(Env.sDnsNameKey ,""), ldtMaintenanceWindowBeginTime, ldtMaintenanceWindowEndTime));
        ...
        public bool bUploadCertToLoadBalancer(string asHash, byte[] abtArrayMinProfile, X509Certificate2 aoOldCertificate, X509Certificate2 aoNewCertificate, string asCertName, string asCertPfxPathFile, string asCertificatePassword)
        {
            ...
            {
                using (X509Certificate2 loLoadBalancerCertificate = new X509Certificate2(asCertPfxPathFile, asCertificatePassword
                        , X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.Exportable))
                {
                    Directory.CreateDirectory(Path.GetDirectoryName(lsLoadBalancerPfxPathFile));
                    File.WriteAllBytes(lsLoadBalancerPfxPathFile, loLoadBalancerCertificate.Export(X509ContentType.Pfx, lsLoadBalancerPassword));
                    DoGetCert.RemoveCertificatePrivateKeyFile(loLoadBalancerCertificate);
                }
            ...
            else
            {
                ...
                lbUploadCertToLoadBalancer = this.bRunPowerScript(moProfile.sValue("-ScriptLoadBalancer", "")
                        ...
                        .Replace("{OldCertificateThumbprint}", null == aoOldCertificate ? "" : aoOldCertificate.Thumbprint)
                        );
                        moProfile.sValue("-ScriptLoadBalancerHelp","Available tokens: {CertificateDomainName}, {LoadBalancerPfxPathFile}, {NewCertificatePfxPathFile}, {NewCertificateThumbprint}, {OldCertificateThumbprint}");
            }
        ...
        public static void ReportErrors(Exception ex, out string asLogFileTextReported)
        {
            ...
            string  lsPreviousErrorsLogPathFile = tvProfile.oGlobal().sRelativeToProfilePathFile(tvProfile.oGlobal().sValue("-PreviousErrorsLogPathFile", "PreviousErrorsLog.txt"));
                    lsPreviousErrorsLogPathFile = Path.Combine(Path.GetDirectoryName(lsPreviousErrorsLogPathFile), String.Format("{0}({1}){2}"
                            , Path.GetFileNameWithoutExtension(lsPreviousErrorsLogPathFile), tvProfile.oGlobal().sValue(Env.sDnsNameKey ,""), Path.GetExtension(lsPreviousErrorsLogPathFile)));
...
certutil -repairstore my {NewCertificateThumbprint}
        ...
        private static bool bDoCheckIn(string asHash, byte[] abtArrayMinProfile)
        {
            ...
            string lsCertName = tvProfile.oGlobal().sValue(Env.sDnsNameKey ,"");
        ...
        private bool bApplyNewCertToSSO(X509Certificate2 aoOldCertificate, X509Certificate2 aoNewCertificate, string asHash, byte[] abtArrayMinProfile)
        {
            ...
            if ( moProfile.bValue("-UseStandAloneMode", true)
                        || !DoGetCert.oDomainProfile.bValue("-IsSsoDomain", false)
                        || moProfile.bValue("-SkipSsoServer", false) )
                return true;
            ...
            bool    lbIsProxy = false;
                    if ( moProfile.ContainsKey("-IsSsoProxy") )
                    {
...
else
{
    ...
    Remove-AdfsCertificate -CertificateType Token-Decrypting       -Thumbprint (Get-AdfsCertificate -CertificateType Token-Decrypting | Where-Object { $_.IsPrimary -eq $false } | Select-Object -First 1).Thumbprint

    Add-AdfsCertificate    -CertificateType Token-Signing          -Thumbprint {NewCertificateThumbprint} -IsPrimary
    Remove-AdfsCertificate -CertificateType Token-Signing          -Thumbprint (Get-AdfsCertificate -CertificateType Token-Signing    | Where-Object { $_.IsPrimary -eq $false } | Select-Object -First 1).Thumbprint
}
...
try {Set-AdfsSslCertificate -Thumbprint ""{NewCertificateThumbprint}""} catch {}
            ...
            string  lsScriptSSO = moProfile.sValue("-ScriptSSO", lsDefaultScript);
                    ...
                    else
                    {
                        lbApplyNewCertToSSO = this.bRunPowerScript(lsScriptSSO
                                ...
                                moProfile.sValue("-ScriptSSOHelp","Available tokens: {CertificateDomainName}, {NewCertificateThumbprint}");
        ...
        private bool bCertificateNotExpiring(string asHash, byte[] abtArrayMinProfile, X509Certificate2 aoOldCertificate)
        {
            ...
            if ( null != aoOldCertificate )
            {
                ...
                if ( lbCheckExpiration )
                {
                    ...
                    {
                        // This allows certificate override uploads to pass through the expiration check.
                        if ( !(Env.sComputerName == lsCertOverrideComputerName && File.Exists(lsCertOverridePfxPathFile)) )
                        {
        ...
        private static tvProfile oHandleUpdates(ref string[] args, out Mutex aoMutex)
        {
            ...
            try
            {
                ...
                string lsCertName = loProfile.sValue(Env.sDnsNameKey, "");
...
"
        ...
        private static tvProfile oHandleIniUpdate(UpdatedEXEs aeExeName, tvProfile aoProfile, string asHash, byte[] abtArrayMinProfile, string[] args)
        {
            ...
            tvProfile   loIniUpdateProfile = new tvProfile();
                        ...
                        using (GetCertService.IGetCertServiceChannel loGetCertServiceClient = Env.oGetCertServiceFactory.CreateChannel())
                        {
                            ...
                            {
                                case UpdatedEXEs.Client:
                                    lsDomainUpdate = Env.sReplaceProfileTokens(loProfile, loGetCertServiceClient.sIniUpdate(asHash, abtArrayMinProfile, lsVersion));
                                    ...
                                    lsServerUpdate = Env.sReplaceProfileTokens(loProfile, loGetCertServiceClient.sIniUpdate(asHash, abtArrayMinProfile, lsServerUpdateVersion));
                                    ...
                                    break;
                                case UpdatedEXEs.GcFailSafe:
                                    lsDomainUpdate = Env.sReplaceProfileTokens(loProfile, loGetCertServiceClient.sFailSafeIniUpdate(asHash, abtArrayMinProfile, lsVersion));
                                    if ( !String.IsNullOrEmpty(lsDomainUpdate) )
                                    {
                                        loIniUpdateProfile.LoadFromCommandLine(lsDomainUpdate, tvProfileLoadActions.Overwrite);
                                        lsNewVersion = loIniUpdateProfile.sValue(lsKeyName, "1");
                                    ...
                                    lsServerUpdateVersion = String.Format(gsUpdateProfileServerFmt, gsUpdateProfileServerKey, aoProfile.sValue(gsUpdateProfileServerKey, DateTime.MinValue.ToShortDateString()));
                                    lsServerUpdate = Env.sReplaceProfileTokens(loProfile, loGetCertServiceClient.sFailSafeIniUpdate(asHash, abtArrayMinProfile, lsServerUpdateVersion));
                                    if ( !String.IsNullOrEmpty(lsServerUpdate) )
                                    {
                                        loIniUpdateProfile.LoadFromCommandLine(lsServerUpdate, tvProfileLoadActions.Merge);
                ...
                if ( lbUpdateSuccess )
                {
                    ...
                    else
                    {
                        tvProfile   loDashboardDataProfile = tvProfile.oProfile(lbtArrayDashboardData);
                                    loDashboardDataProfile["-JSON"] = loDashboardDataProfile.sJson("-JSON");
        ...
        {
            bool        lbHostLicenseAccepted = aoProfile.bValue("-HostLicenseAccepted", false);
            ...
            {
                bool    lbHostStopped = true;   // Consider it "stopped" even if not previously running.
                        DoGetCert.sStopHostExePathFile();

                lbRestartHost = true;

                // Write the updated EXE (if any).
                if ( lbHostStopped && null != lbtArrayGpcExeUpdate )
                {
                    try
                    {
                        // Write the EXE.
                    ...
                    catch (IOException)
                    {
                        ...
                        Env.LogIt(String.Format("Another host update appears to be already running. Will retry later."
                                                    , FileVersionInfo.GetVersionInfo(lsHostExePathFile).FileVersion));
                        Env.ScheduleOnErrorBounce();
                    }
                    catch (Exception ex)
                ...
                if ( lbHostStopped && !String.IsNullOrEmpty(lsHostIniUpdate) )
                {
                    ...
                    else
                    {
                        ...
                        loUpdateProfile.Remove("-RemoveTasks");
                        loUpdateProfile.Remove("-BackupSet");
                        loUpdateProfile.Remove("-CleanupSet");

                        // Finally, merge in the update (sans keys already handled above).
            ...
            if ( lbRestartHost )
            {
                bool lbClientSetupDone = "" != aoProfile.sValue("-ContactEmailAddress" ,"") && "" != aoProfile.sValue(Env.sDnsNameKey ,"");
                ...
                if ( lbClientSetupDone && lbHostLicenseAccepted )
                {
                    ...
                    {
                        if ( null != loHostProcess && null != loHostProcess.StartInfo )
                            Env.LogIt(String.Format("Host process (\"{0}\") could not be started. Try using -HostFetched.", loHostProcess.StartInfo.FileName));
        ...
        {
            if ( aoProfile.bValue("-DisableSetupCertChanges", false) )
                return;
        ...
        public bool bGetCertificate()
        {
            ...
            try
            {
                string[]            lsSanArray = moProfile.oProfile("-SanList").sOneKeyArray("*");
                                    ...
                                    {
                                        lsSanArray[0] = moProfile.sValue(Env.sDnsNameKey ,"");
                                    }
                                    else
                                    if ( lsSanArray.Length > 1 )
                                    {
                                        moProfile[Env.sDnsNameKey] = lsSanArray[0];
                ...
                lsCertName = moProfile.sValue(Env.sDnsNameKey ,"");
                    ...
                    if ( lbGetCertificate && !this.bMainLoopStopped )
                    {
                        ...
                        using (GetCertService.IGetCertServiceChannel moGetCertServiceClient = Env.oGetCertServiceFactory.CreateChannel())
                        {
                            ...
                            else
                            {
                                ...
                                if ( lbGetCertificate && !this.bMainLoopStopped && "" != DoGetCert.oDomainProfile.sValue("-LoadBalancerComputerName", "")
                                        && !lbLoadBalancerCertPending && !lbLoadBalancerCertReady )
                                {
                                    ...
                                    if ( lbCertOverrideApplies )
                                        ...
                                        lbGetCertificate = this.bUploadCertToLoadBalancer(lsHash, lbtArrayMinProfile, loOldCertificate, loLoadBalancerCert, lsCertName, lsCertPfxPathFile, lsCertificatePassword);
                ...
                if ( lbGetCertificate && !this.bMainLoopStopped && null == lbtArrayNewCertificate && (!lbRepositoryCertsOnly || lbCertOverrideApplies) )
                {
                    if ( lbCertOverrideApplies )
                    {
                        ...
                        if ( DoGetCert.oDomainProfile.bValue("-CertOverridePfxReady", false) )
                        {
                            Env.LogIt("The certificate override is ready, but it hasn't yet been released for download. Check the SCS access log for details.");
                    ...
                    else
                    {
                        ...
                        // Open SSL (together with -ScriptAcmeExported) allows for changes to
                        // the certificate upon arrival from the certificate provider network.
...
Get-ACMEServiceDirectory ""{AcmeWorkPath}"" -ServiceName ""{AcmeServiceName}"" -PassThru
                                            ...
                                            moProfile.sValue("-ScriptStage1Help","Available tokens: {AcmePsPath}, {AcmePsVer}, {AcmeServiceName}, {AcmeSystemWide}, {AcmeWorkPath}, {SanPsStringArray}");
                            }
                        }
...
New-ACMEOrder ""{AcmeWorkPath}"" -Identifiers {SanPsStringArray}
                                        ...
                                        moProfile.sValue("-ScriptStage2Help","Available tokens: {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {SanPsStringArray}, {UseNewAccount}");
...
$challenge | Complete-ACMEChallenge ""{AcmeWorkPath}""
                                                    ...
                                                    moProfile.sValue("-ScriptStage3Help","Available tokens: {AcmeChallengePath}, {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {HttpChallengeSleepSecs}, {SanArrayIndex}, {SanPsStringArray}");
                                    }
                                    else
                                    {
                                        if ( lbSingleSessionEnabled )
...
$challenge | Complete-ACMEChallenge ""{AcmeWorkPath}""
                                                    ...
                                                    moProfile.sValue("-ScriptStage3DnsHelp","Available tokens: {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {DnsChallengeSleepSecs}, {SanArrayIndex}, {SanPsStringArray}");
                                    }
                                }
                            }
                        }
...
$order | Update-ACMEOrder ""{AcmeWorkPath}"" -PassThru
                                            ...
                                            moProfile.sValue("-ScriptStage4Help","Available tokens: {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {SanPsStringArray}");
                                ...
                                if ( !lsOutput.Contains(lsSubmissionPending) )
                                {
                                    ...
                                    if ( !lbGetCertificate )
                                    {
                                        ...
                                        {
                                            Env.LogIt("");
                                            Env.LogIt("Note: -UseNonIISBindingOnly=True. That means IIS is not used in this context. You can't setup an HTTP challenge without it.");
                                        }
                                    }
...
for ($i=0; $i -lt $authZ.Length; $i++)
{
    ...
    echo Remove DNS challenge command-line goes here (-Name $challenge.Data.TxtRecordName -Value $challenge.Data.Content -Remove).
                                        ...
                                        moProfile.sValue("-ScriptDnsCleanupHelp","Available tokens: {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {SanPsStringArray}");
                        }
...
Complete-ACMEOrder      ""{AcmeWorkPath}"" -Order $order -CertificateKey $certKey
                                        ...
                                        moProfile.sValue("-ScriptStage5Help","Available tokens: {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {SanPsStringArray}");
                        }
...
$order | Update-ACMEOrder ""{AcmeWorkPath}"" -PassThru
                                            ...
                                            moProfile.sValue("-ScriptStage6Help","Available tokens: {AcmePsPath}, {AcmeSystemWide}, {AcmeWorkPath}, {SanPsStringArray}");
...
Export-ACMECertificate  ""{AcmeWorkPath}"" -Order $order -CertificateKey $certKey -Path ""{CertificatePathFile}""
                            ...
                            if ( lbGetCertificate && !this.bMainLoopStopped )
                            {
                                string  lsScriptAcmeExported = moProfile.sValue("-ScriptAcmeExported", "");
                                        if ( !String.IsNullOrEmpty(lsScriptAcmeExported) )
                                        {
                                            Env.LogIt("");

                                            lbGetCertificate = this.bRunPowerScript(lsScriptAcmeExported
                                                        .Replace("{AcmeWorkPath}", lsAcmeWorkPath)
                                                        .Replace("{CertificateFile}", lsCertificateFile)
                                                        .Replace("{CertificatePassword}", lsCertificatePassword)
                                                        .Replace("{CertificatePathFile}", lsCertPfxPathFile)
                                                        );
                                                        moProfile.sValue("-ScriptAcmeExportedHelp","Available tokens: {AcmeWorkPath}, {CertificateFile}, {CertificatePassword}, {CertificatePathFile}");
                                        }
                            }
                        }
                    }
                ...
                if ( lbGetCertificate && !this.bMainLoopStopped )
                {
                    ...
                    if ( !lbGetCertificate )
                    {
                        Env.LogIt(Env.sExceptionMessage(String.Format("The new certificate PFX file (\"{0}\") could not be found!", lsCertPfxPathFile)));
                    }
                    else
                    {
                    ...
                    if ( lbGetCertificate && !this.bMainLoopStopped && null == lbtArrayNewCertificate && !moProfile.bValue("-UseStandAloneMode", true) )
                    {
                        // Upload new certificate to the load balancer.
                        if ( lbGetCertificate && !this.bMainLoopStopped && "" != DoGetCert.oDomainProfile.sValue("-LoadBalancerComputerName", "") )
                            lbGetCertificate = this.bUploadCertToLoadBalancer(lsHash, lbtArrayMinProfile, loOldCertificate, loNewCertificate, lsCertName, lsCertPfxPathFile, lsCertificatePassword);
                        ...
                        {
                            Env.LogIt("");
                            Env.LogIt(String.Format("New certificate thumbprint: {0} (\"{1}\")", loNewCertificate.Thumbprint, Env.sCertName(loNewCertificate)));
                            ...
                            string  lsScriptCertAcquired = moProfile.sValue("-ScriptCertAcquired", "");
                                    if ( !String.IsNullOrEmpty(lsScriptCertAcquired) )
                                    {
                                        ...
                                        this.bRunPowerScript(lsScriptCertAcquired
                                                ...
                                                .Replace("{NewCertificatePfxPathFile}", lsCertPfxPathFile)
                                                ...
                                                moProfile.sValue("-ScriptCertAcquiredHelp","Available tokens: {CertificateDomainName}, {NewCertificatePfxPathFile}, {NewCertificateThumbprint}, {OldCertificateThumbprint}");
                                    }
                        }
                    }
                }
                ...
                if ( lbGetCertificate && !this.bMainLoopStopped && (null != lbtArrayNewCertificate || moProfile.bValue("-UseStandAloneMode", true)) )
                {
                    ...
                    if ( (null != loOldCertificate && loOldCertificate.Thumbprint != loNewCertificate.Thumbprint) || null == loOldCertificate )
                    {
                    ...
                    if ( lbGetCertificate && !this.bMainLoopStopped )
                    {
                                ...
                                && (moProfile.bValue("-UseNonIISBindingOnly", false) || moProfile.bValue("-UseNonIISBindingAlso", false)) )
                        {
                            ...
                            lbGetCertificate = Env.bRunPowerScript(moProfile.sValue("-ScriptNonIISBinding", "")
                                                                    ...
                                                                    moProfile.sValue("-ScriptNonIISBindingHelp","Available tokens: {CertificateDomainName}, {NewCertificatePfxPathFile}, {NewCertificateThumbprint}, {OldCertificateThumbprint}");
                            ...
                            string  lsScriptBindingDone = moProfile.sValue("-ScriptBindingDone", "");
                                    if ( !String.IsNullOrEmpty(lsScriptBindingDone) )
                                    {
                                        ...
                                        this.bRunPowerScript(lsScriptBindingDone
                                                ...
                                                moProfile.sValue("-ScriptBindingDoneHelp","Available tokens: {CertificateDomainName}, {NewCertificateThumbprint}, {OldCertificateThumbprint}");
                                    }
                        }
